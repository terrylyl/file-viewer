...
- 支持拖拽表头调整列顺序。
- 列数较少时保持列自身宽度左对齐展示，不强制拉伸到全宽。
- 表头筛选菜单支持重命名任意列，并可单列恢复原始列名。
- 支持单元格自动换行全文显示，普通摘要截断阈值为 500 字。
- 右侧详情与弹窗默认分块加载 100000 字。
- 筛选后显示当前行数统计。
- 表头筛选菜单可隐藏当前结果行，也可以恢复隐藏行。
- 支持新增列：可自定义列名，内容可为空、序列号（从 1 开始）或复制已有列；自定义列可在表头菜单中删除。
- 支持拼接列：可按顺序选择多列、设置别名，生成保留 Markdown 三反引号代码块格式的单列；拼接列作为自定义列追加到最右侧，可复用自定义列删除。
