...
- CSV 普通路径与大文件路径均采用严格 RFC 4180 优先解析；只有列数异常的记录才进入宽容修复，避免兼容逻辑改写合法 CSV。
- 宽容修复会从异常记录起点局部重读，并且只有结果列数与表头一致时才采纳；修复失败时保留严格解析结果并报告异常。
- 新增普通路径与大文件路径的跨路径一致性测试、畸形 CSV 模糊测试和合法 CSV 的 RFC 4180 差分测试，持续校验两条解析路径的表头和数据一致。
- 导入设置支持「严格 CSV」模式，可关闭 JSON、Markdown 代码块、反斜杠等非标准 CSV 的兼容解析。
