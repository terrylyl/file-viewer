...
- 分隔符探测支持标题行、导出说明、注释行和前导空记录，并防止正文 Markdown 表格中的大量 `|` 反超真实分隔符；单行无结尾换行文件也能正确识别。
- 表头可从前 3 条单格标题行之后自动识别；导入设置支持手动指定分隔符和表头行，低置信度的一列解析会给出可恢复提示。
- `.tsv` 文件固定使用 Tab 分隔符；CSV 仍允许自动识别逗号、分号、Tab 或竖线。
- 超长 cell 大文件的可视区预览采用有界并发读取，并去重滚动中的重复请求，避免网格可点击但长时间显示空白。
...
- 300 MiB 以上的 CSV/TSV/TXT/JSONL 使用原文件偏移索引与按需 `File.slice()` 读取，当前上限为 500 MiB，不再向 OPFS 复制完整文件。
...
- 超过 300 MiB 的 CSV/TSV/TXT/JSONL 会启用大文件模式：Worker 流式扫描原始 `File` 并建立 cell 字节偏移，主线程只缓存当前视口预览及少量按需读取内容；文件内容仍不会离开浏览器。
...
- CSV/TSV/TXT/JSONL 在 300 MiB 以上自动切换到大文件模式，当前支持到 500 MiB。大文件模式以字节偏移索引代替完整文本物化，与普通 CSV 使用同一套复杂字段解析规则，正式支持最新版 Chrome/Edge 桌面端，并可在读取阶段取消。
...
- 小型数据集仍使用内存行数组；CSV/TSV/TXT/JSONL 超过 300 MiB 时，大文件 Worker 只保存原始 `File` 的 cell/行字节偏移和短预览，主线程使用按字节限制的预览、完整 cell 与完整行缓存。
