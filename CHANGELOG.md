...
- 导入告警改为反映最终结果而不是中间那遍严格解析：含未加引号 JSON 或围栏的文件在严格视角下必然「引号未闭合」，只要修复之后每行列数都对上了就不再提示；修复一次都没动手时仍照原样报出。
...
- 新增跨路径一致性测试 `tests/csv-path-parity.test.mjs`：同一份文件分别走普通路径与大文件字节索引器，逐形态比对编码、分隔符、表头、每一行与告警类型，覆盖 43 种形态（四种分隔符、BOM、CRLF、老式 ``、标题行与前言行、公式、Windows 路径、引号内换行与双写、未加引号 JSON、跨行 pretty-print、Markdown 围栏与表格、GB18030、截断 JSON、超长 cell 等）。两条解析器各自手写、各自演化，这一轮至少四个 bug 是它们不一致造成的，此前只能靠临时脚本发现。
