...
- 导入设置新增「严格 CSV」开关：完全按 RFC4180 解析，关掉 JSON、Markdown 代码块、反斜杠转义等全部宽容处理，结果与 Python `csv` 一致。宽容解析对畸形文件是兜底，但它对合法文件也生效，判断出错时此前只能改数据源；现在一键就能退回确定的解析路径。它是解析方式的偏好而非某个文件的属性，因此换文件不重置（与编码选择一致）。
- 新增差分模糊测试 `tests/csv-differential-fuzz.test.mjs`：随机生成合法 CSV 与严格 RFC4180 参照实现比对，失败时自动最小化用例。两条性质——严格模式逐字段等价、宽容模式不减少记录数——各跑 1500 组。固定语料只能守住已知形态，这一轮线上暴露的两个丢行 bug 都在它的覆盖范围内。

### Fixed

- 修复引号内的 `\"` 被当作转义、吃掉字段收尾引号并把下一行并进当前记录的问题。RFC4180 在引号内没有歧义（`"` 必须双写、`\` 是普通字符），`"C:\dir\",next` 按标准规则本来就解析得对，这条特例反而在合法 CSV 上改变了记录数——差分模糊测试可稳定复现。
- 修复未加引号的 `\` 吞掉分隔符后，下一个字段的开引号退化成普通字符、字段内换行随之失去保护、整条记录被拆散的问题。`\` 后的分隔符现在多看一个字符：后面紧跟开引号就按真正的分隔符处理，`C:\dir\,next` 这类路径的合并行为不变。
- 修复 24–299 MiB 的长文本 CSV 因大文件切换点被误当成容量上限、提高到 300 MiB 后改走普通内存解析路径，导致未规范引用的 Markdown 换行被拆成大量记录的问题。流式偏移索引路径恢复从 24 MiB 启用；支持上限仍为 500 MiB。
- 修复合法 CSV 中引号字段以 `[` / `{` 开头但内容并非完整 JSON 时，后续记录里的闭括号被误认为当前结构收尾、从而把数十条真实记录合并成一条的问题。无效的引号内结构一旦跨越物理行就回退到标准 CSV 边界；同一行内双重编码 JSON 的兼容行为保留。
- 修复外层加了 CSV 引号、内部引号却没有双写的 JSON 数组（内容形如 `"[\n\t\t\t"BOLL-DMI",…]"`，缩进是字面转义序列的双重编码内容）在第一个 `",` 处被当成字段收尾，后面的元素被挤进 `Column 29`、`Column 30` 等后续列。2.3.5 能完整读出，2.3.7 加入 JSON 前瞻白名单后丢失。引号内的开括号现在放宽前瞻与 JSON 校验：字段边界本来就由引号定界，`[2024上半年` 吞掉整个文件那个风险是未加引号场景独有的。
- 修复未加引号的 pretty-print JSON（`[` 或 `{` 后面直接换行缩进）被按换行切成好几行：开括号后的换行和回车现在与空格、Tab 一样按 JSON 缩进对待。此前只认空格和 Tab，同样一份 JSON 用 Tab 缩进能保住、用换行缩进就散架。前瞻不通过时改为回滚重放，不会把等待期间吞掉的换行连同记录边界一起吃掉。
- 修复跨行 JSON 的续行（`"a": 1,` 这种行尾停在分隔符上的行）被误判成一条完整记录，导致 2 列表里的多行 JSON 被拆开。首行本来就有这条保护，现在后续行也一样。
- 修复大文件模式下未加引号的公式（`=IF(A1 > 0, B1, C1)`）被逗号切成多列、并多出「列数不一致」告警：字节索引器现在按普通路径同一套括号配平算法把切开的 descriptor 合并回去。此前公式修复只存在于普通路径，同一份文件跨过 24 MiB 阈值结果会变。
...
- 字段内出现没有双写的 `"` 且确实切出了列数不一致的行时，导入异常里给出「疑似引号未双写」，直接指向生成端而不是让人怀疑本地解析。合法文件（引号已双写）不会误报。
