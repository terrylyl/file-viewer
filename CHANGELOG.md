...
- 新增分隔符探测回归语料 `tests/delimiter-corpus.test.mjs`，覆盖四种分隔符、标题行/前言行/注释行、前导空行与占位行、Markdown 表格与标签列干扰、无结尾换行等约 40 种形态；已知待修的前言行用例在语料里标记为 pending。

### Known Issues

- 首条非空记录里不含真分隔符时（`月度报表` 后面才是 `id;name;age`），`;`、`|`、Tab 仍会回退成逗号并塌成一列，此时会给出上面的告警。修复需要同时放宽首记录硬淘汰并加入空首尾字段降权，见语料中的 pending 用例。
- 即使分隔符判断正确，标题行本身仍会被当作表头；表头行识别是独立课题。
