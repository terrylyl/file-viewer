...
- 版本号硬编码在该契约测试里（`app package version is 2.3.7`）。升版本要同时改 `package.json`、`sbom.cdx.json` 的 `metadata.component.version`、这条测试、`README.md`/`CHANGELOG.md`/`SECURITY.md`；tag `v*` 与 `package.json` 版本不一致会让 release job 失败。
